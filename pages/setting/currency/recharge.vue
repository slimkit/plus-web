<template>
  <div class="p-setting-currency-recharge">
    <Loadmore
      ref="loader"
      @refresh="onRefresh"
      @loadmore="onLoadmore"
...
export default {
  ...
  methods: {
    async onRefresh () {
      const params = { limit, action: 'rechage', type: 1 }
      const list = await this.$axios.$get('/currency/orders', { params })
      this.list = list
      this.loader.afterRefresh(list.length < limit)
    },
    async onLoadmore () {
      const params = { limit, action: 'rechage', type: 1, after: getLastField(this.list) }
      const list = await this.$axios.$get('/currency/orders', { params })
      this.list.push(...list)
